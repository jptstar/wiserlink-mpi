...
class WiserLinkCoordinator(DataUpdateCoordinator[dict]):
        ...
        await self._gas_store.async_save(
            {
                ...
                "automatic_reboot_disabled": True,
            }
        )
    ...
    async def async_reboot(self, reason: str = "manuel", automatic: bool = False) -> None:
        """Reboot the MIP only for an explicit/manual request."""
        if automatic:
            raise WiserLinkError(
                "Le redémarrage automatique lié à la dérive gaz est désactivé "
                "tant que le comportement du MPE n’est pas validé"
            )
    ...
    def gas_monitor_data(self) -> dict[str, Any]:
        """Return observation-only gas timing diagnostics."""
        ...
        return {
            ...
            "drift_minutes": drift,
            "drift_exceeded": (
                abs(drift) > self._gas_tolerance_minutes
                if self._gas_drift_control and drift is not None
                else None
            ),
            "target_time": self._gas_target_time,
            "tolerance_minutes": self._gas_tolerance_minutes,
            "observation_enabled": self._gas_drift_control,
            "automatic_reboot": False,
            "automatic_reboot_reason": (
                "désactivé : comportement de recalage du MPE non validé"
            ),
